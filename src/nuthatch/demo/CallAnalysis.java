...
public class CallAnalysis {
	...
	public static ParseTable getJavaParseTable() throws ParseError,
			IOException, InvalidParseTableException {
		InputStream stream = JavaPatterns.class
				.getResourceAsStream("syntax/JavaCompilationUnit-15.tbl");
	 ...
	 */
	public static void main(String[] args) throws TokenExpectedException,
			BadTokenException, ParseException, ParseError, SGLRException,
			IOException, InvalidParseTableException {
		IStrategoTerm term = StrategoAdapter.parseFile(
				"/tmp/RationalValue.java", getJavaParseTable());
		...
		System.out.println("aspectVariant(t).equals(ancestorVariant(t)) = "
				+ resultMap.equals(resultMap2));
	}
	...
	private static MultiMap<String, String> ancestorVariant(IStrategoTerm term) {
		...
		Strategy<TermEngine> calls = new Visitor<TermEngine>() {
			...
			public void onEntry(TermEngine e) {
				Environment<TreeCursor<IStrategoTerm, Integer>> env = EnvironmentFactory
						.env();
				if (Invoke(Method(MethodName(var("name"))), var("args")).match(
						e, env)) {
					resultMap.add(getSurroundingMethodName(e),
							nameToStr(env.get("name")));
				}
			}

		};
		TermEngine engine = new TermEngine(StrategoAdapter.termToTree(term),
				calls);
		engine.engage();
	...
	private static MultiMap<String, String> aspectVariant(IStrategoTerm term) {
		...
		Visitor<TermEngine> visitor = new Visitor<TermEngine>() {
			...
			public void onEntry(TermEngine e) {
				...
				if (Invoke(Method(MethodName(var("name"))), var("args")).match(
						e, env)) {
					resultMap.add(nameToStr(e.getSubtreeVar(scopeName)),
							nameToStr(env.get("name")));
				}
			}
		};

		Strategy<TermEngine> nameTracker = new VisitorAspect<TermEngine>(
				visitor) {
			...
			public void beforeEntry(TermEngine e) {
				...
				if (MethodDec(var("head", MethodDecHead(_, _, _, name, _, _)),
						_).match(e, env)) {
					e.setSubtreeVar(scopeName, name.get());
				} else if (ConstrDec(
						var("head", ConstrDecHead(_, _, name, _, _)), _).match(
						e, env)) {
					e.setSubtreeVar(scopeName, name.get());
				}
			}
	...
	private static String getSurroundingMethodName(TermEngine e) {
		Environment<TreeCursor<IStrategoTerm, Integer>> env = EnvironmentFactory
				.env();
		if (ancestor(
				or(MethodDec(MethodDecHead(_, _, _, var("scopeName"), _, _), _),
						ConstrDec(ConstrDecHead(_, _, var("scopeName"), _, _),
								_))).match(e, env)) {
	...
	static class MultiMap<K, V> {
		...
		public int hashCode() {
			...
			int result = 1;
			result = prime * result
					+ ((resultMap == null) ? 0 : resultMap.hashCode());
			return result;
		}
	}
}
